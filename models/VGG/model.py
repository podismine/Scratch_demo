...
class VGG(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.features(x)
        # N x 512 x 7 x 7
        x = torch.flatten(x, start_dim=1)##第0个维度是batch维度，所以从第一个维度开始
        # N x 512*7*7
        x = self.classifier(x)
        return x
...
def make_features(cfg: list):##特征网络结构
    layers = []
...
def vgg(model_name="vgg16", **kwargs): ##实例化vgg模型
    try:
        cfg = cfgs[model_name]
    except:
